...
import type {
  Organization,
  OrganizationDataSnakeCase,
} from "@/interfaces/Organization";
import { useEffect, useState } from "react";

const usetransformOrganizationData = (
  data: OrganizationDataSnakeCase | null
) => {
  const [transformedData, setTransformedData] = useState<Organization | null>(
    null
  );
...
const toOrganization = (organizationData: OrganizationDataSnakeCase) => {
...
function transformOrganizations(data: OrganizationDataSnakeCase[]) {
  const result = [];
